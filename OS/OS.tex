...
\section{Memory Management}
	...
	\subsection{Memory address, Swapping and Managing Free Memory Space}
		\begin{enumerate}
			...
			\lb Explain free disk space management with example. \hfill [4] (\bo{71 Bh})

			\item Explain how operating system manages free blocks of Secondary Storage. \hfill [6] (\bo{72 Ash}, 73 Ma)
		\end{enumerate}
	...
	\subsection{Multiprogramming with Fixed \& Variable Partition}
		\begin{enumerate}
			\item What are the differences of fixed and variable partitioning system of memory for multiprogramming? \hfill [3] (\bo{75 Bh})
		\end{enumerate}
	...
	\subsection{Virtual Memory Management}
		\begin{enumerate}
			...
			\item What is the role of TLB? \hfill [2] (\bo{71 Bh})
	...
	\subsection{Memory Allocation Techniques}
		\begin{enumerate}
			...
			\item (Assumed) Differentiate between internal and external fragmentation? \hfill [2] (\bo{79 Ch})

			\item Define internal and external fragmentation. \hfill [2] (75 Ba)

			\item What is (Explain) DMA? \hfill [2] (\bo{79 Ch, 70 Bh}) [4] (\bo{67 Mng}) [5] (68 Ma)
		...
		\subsubsection{Non-Contiguous: Paging/Segmentation}
			\begin{enumerate}
				...
				\item Prepare a comparative note on the followings: a. Virtual memory management using Paging versus Segmentation. \hfill [4] (\bo{76 Bh})
	...
	\subsection{Page Replacement Algorithms}
		\begin{enumerate}
			...
			\lb With example, show that FIFO page replacement algorithm suffers from Belady's anomaly. \hfill [3] (\bo{73 Bh})
	...
	\subsection{Thrashing}
		\begin{enumerate}
			...
			\item Write short notes on: Thrashing. \hfill [3] (\bo{\texttt{81 Bh, 80 Bh}}) [4] (\bo{75 Bh})
		\end{enumerate}
	\subsection{Numericals}
		\begin{enumerate}
			...
			\item Consider the following page reference Strings; 2, 3, 4, 2, 1, 3, 7, 5, 4, 3, 1, 5. Find how many page faults occur according to optimal, LRU and LFU page replacement algorithm assuming 3-page frames. \hfill [6] (\bo{80 Ch})		
...
\section{File Systems}
	...
	\subsection{File: Name, Structure, Types, Access, Attribute, Operations}
		\begin{enumerate}
			...
			\item What is file attribute? \hfill [1] (\bo{79 Ch}, \texttt{81 Ba}, 75 Ba, 73 Ma, 71 Ma)
			...
			\item What are the major differences between file system interfaces and file system implementation? \hfill [4] (\bo{70 Bh})
		\end{enumerate}
	...
	\subsection{File System Implementation}
		...
		\subsubsection{Implementing File: Contiguous Allocation, Link List Allocation, Link List Allocation with Table, Inode}
			\begin{enumerate}
				...
				\item Explain I-node approach of file implementation with its adv and disadv. \hfill [5] (\bo{\texttt{81 Bh}}) [6] (76 Ba) [8] (75 Bh)

				\item What is I-node? \hfill [3] (\bo{67 Mng})

				\item Explain about I-node and file system backup. \hfill [5] (\bo{68 Bh})

				\item (Assumed) Write short ntoes on: Compaction. \hfill [3] (\bo{\texttt{81 Bh}})
				
				\item Write short notes on: Compaction and Coalescing. \hfill [3] (\bo{80 Ch})
				\lb Differentiate Compaction and Coalescing. \hfill [2] (\bo{77 Ch, 73 Bh}) [2.5] (70 Ma) [4] (\bo{\texttt{79 Bh}, 71 Bh})
				\lb Difference with example. \hfill [4] (75 Ba)

				\item Suggest which file organization technique is most appropriate for "tape storage". \hfill [1] (\bo{79 Ch})
	...
	\subsection{Mapping File Blocks on The Disk Platter}
		\begin{enumerate}
			\item (Assumed) Why are output files for the printer normally spooled on disk before being printed? \hfill [2] (\bo{80 Ch})

			\item (Assumed) Prepare a comparative note on: Spooling versus Deadline Scheduling. \hfill [4] (\bo{76 Bh})
		\end{enumerate}
	...
	\subsection{File System Performance}
		\begin{enumerate}
			...
			\item How do you measure the file system performance and how can it be improved? \hfill [4] (\bo{\texttt{79 Bh}})
		\end{enumerate}
...
\section{I/O Management and Disk Scheduling}
	...
	\subsection{Principles of I/O software}
		\begin{enumerate}
			...
			\lb Short notes on principles of IO software. \hfill [3.5] (\bo{73 Bh})
			...
			\item What are the functions of device independent IO software? \hfill [2] (\bo{67 Mng}, \texttt{80 Ba}) [4] (\bo{70 Bh})
	...
	\subsection{Numericals}
		\begin{enumerate}
			...
			\item Consider a disk drive having 100 cylinders. The head is currently serving request at cylinder 43 and the previous request was at 56. The queue of pending request is 86, 70, 13, 74, 48, 9, 22, 50, 30. Starting from current head position, what is total head movement (in cylinders) to service the pending requests of each of following disk arm scheduling algorithms. \hfill [6] (\bo{\texttt{81 Bh}})\\
			a. FCFS\\
			b. SSTF\\
			c. SCAN\\
			d. LOOK
...
\section{Deadlock}
	...
	\subsection{Principles of deadlock}
		\begin{enumerate}
			...
			\item Explain necessary conditions for deadlock. \hfill [2] (\bo{80 Ch}, \texttt{81 Ba}, 75 Ba) [3] (\bo{74 Bh}) [4] (\bo{72 Ash, 69 Bh}, 71 Ma)
	...
	\subsection{Deadlock Detection}
		\begin{enumerate}
			...
			\item Consider the system consists of m resources of same type being shared by n process of which need at most two resource. Explain whether the system is deadlock free or not? \hfill [4] (\bo{67 Mng})
		\end{enumerate}
	...
	\subsection{Numericals}
		\begin{enumerate}
			\item A system has 2 process and 3 resources. Each process needs maximum of two resources, is deadlock possible? Explain. \hfill [2] (\bo{74 Bh})
			...
			\item Consider a system with 5 concurrent processes (P0, P1, P2, P3, P4) and 4 resource types (R0, R1, R2, R3). The number of instances of each resource type in the system are 6, 4, 4, 2 respectively. Allocation table and maximum claim table are as follows: \hfill [7] (\bo{73 Bh}) [8] (\bo{\texttt{80 Bh}, 76 Bh})
...
\section{Security}
	...
	\subsection{Basics of Cryptography}
		\begin{enumerate}
			...
			\item Write short notes on Caesar Cipher. \hfill [3] (\bo{\texttt{79 Bh}, 74 Bh})
	...
	\subsection{Protection Mechanisms}
		\begin{enumerate}
			...
			\lb Explain protection domain. \hfill [2.5] (\bo{69 Ch})
			...
			\item (Assumed) Why 'HASH' function is called Message Digestor? \hfill [1] (\bo{79 Ch}) [2] (\bo{\texttt{80 Bh}, 77 Ch})

			\item (Assumed) What are the roles of system administrator for change management? \hfill [4] (\bo{78 Ch})

			\item Explain how you can implement security and protection on all components of a system. \hfill [6] (\bo{72 Ash})
	...
	\subsection{Authentication}
		\begin{enumerate}
			\item How authentication is an essential mechanism for maintaining security? Explain. \hfill [4] (\bo{74 Bh})
		\end{enumerate}

	\subsection{OS Design Considerations For Security}
	\subsection{Access Control Lists And OS Support}
		\begin{enumerate}
			\item What is ACL? \hfill [2] (\bo{\texttt{81 Bh, 80 Bh}, 79 Ch, 77 Ch, 70 Bh}, \texttt{81 Ba}) [2.5] (\bo{69 Ch}, \texttt{80 Ba}) [3] (\bo{68 Bh})
...
\section{System administration}
	...
	\subsection{Administration Tasks}
		\begin{enumerate}
			...
			\item What are the roles of a system administrator? \hfill [2.5] (\texttt{81 Ba}) [3.5] (\bo{73 Bh}) [4] (\bo{\texttt{81 Bh}, 76 Bh, 70 Bh}, 75 Ba, 72 Ma, 70 Ma) [5] (\bo{80 Ch, 71 Bh}) [6] (\bo{\texttt{80 Bh}})
			...
			\lb ... adminstrator to keep system updated and efficient. Explain with example. [3+5] (\bo{75 Bh})
