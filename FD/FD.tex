...
\SetWatermarkText{{\fontsize{45}{0}\selectfont Creation of PUL079BEI008}}
\SetWatermarkScale{1.4}
\SetWatermarkColor{blue!25}

\pagestyle{fancy}
\fancyhf{}
\renewcommand{\headrulewidth}{0pt}
\renewcommand{\footrulewidth}{0pt}
\fancyfoot[L]{Contact \& queries: waraunika2956@gmail.com, 079bei008.arun@pcampus.edu.np}
\fancyfoot[R]{\thepage}
...
\section{Introduction}
	...
	\subsection{Numericals of EX704}
		\begin{enumerate}
			...
			\includegraphics[width=4in]{./pics/fd_5}
		\end{enumerate}
	...
	\begin{center}(8 Hours/14 Marks)\end{center}
		...
		\subsection{Butterworth response, Butterworth pole locations, Butterworth filter design from specifications}
			\begin{enumerate}
				...
				a. \w$_p$ = 2000rad/s, \w$_s$ = 3000rad/s \hspace{1.5cm}
				$\alpha_{max}$ = 1dB, $\alpha_{min}$ = 12dB \hfill[3] (\bo{\texttt{79 Bh}})\\
				b. \w$_p$ = 2000rad/s, \w$_s$ = 3000rad/s \hspace{1.5cm}
				$\alpha_{max}$ = 0.5dB, $\alpha_{min}$ = 22dB \hfill[3] (\bo{78 Bh})\\
				c. \w$_p$ = 1000rad/s, \w$_s$ = 2000rad/s \hspace{1.5cm}
				$\alpha_{max}$ = 1dB, $\alpha_{min}$ = 20dB \hfill[3] (\bo{75 Ch})\\
				d. \w$_p$ = 200rad/s, \w$_s$ = 2000rad/s \hspace{1.7cm}
...
\section{Frequency transformation}
	...
	\subsection{Lowpass to highpass transformation}
		\begin{enumerate}
			...
			\includegraphics[width=4in]{./pics/fd_9}
		\end{enumerate}
	...
	\subsection{Lowpass to bandpass transformation}
		\begin{enumerate}
			...
			\item Obtain the BPF from LPF given in the table having center frequency 10\textsuperscript{4} rad/s and bandwidth of 9.9 x 10\textsuperscript{4} rad/s. \hfill[4] (\bo{73 Ch})
	...
	\subsection{Lowpass to bandstop transformation}
		\begin{enumerate}
			...
			\includegraphics[width=4in]{./pics/fd_6}
	...
	\subsection{One-port passive circuits}
		...
		\subsubsection{Synthesis of LC one-port circuits, Foster and Cauer circuits}
			\begin{enumerate}
				...
				\lb Foster series and cauer I methods. \hfill [2+3+3] (\bo{80 Bh})
...
\section{Design of Resistively-Terminated Lossless Fitter}
	...
	\subsection{Synthesis of LC ladder circuits }
		\begin{enumerate}
			...
			\item Design a Band pass filter having center frequency at 1500 rad/sec and bandwidth 300 rad/sec from a 4\textsuperscript{th} order Butterworth low pass resistively terminated lossless filter. \textit{[\hyperref[sec:tables_81bh]{Refer Table]}}. \hfill [4] (\bo{\texttt{81 Bh}})
	...
	\subsection{Second order active sections (biquads)}
		...
		\subsubsection{Sallen-Key biquad circuit and Multiple-feedback biquad (MFB) circuit}
			\begin{enumerate}
				...
				\item Design a 4\super{th} order Butterworth LPF using cascaded two Sallen-Key biquads having half power frequency of 1 kHz and largest capacitor of 0.1 $\mu$F in your final circuit. \hspace{3cm} [8] (\bo{79 Bh})
		...
		\subsubsection{RC-CR transformation}
			\begin{enumerate}[topsep=0pt]
				\item Explain RC-CR transformation. \hfill [1] (\bo{70 Ch}, 75 Ash)
				\lb with suitable examples. \hfill [2] (79 Ba) [4] (\bo{73 Ch})

				\item How can you convert Sallen Key low pass filter into Sallen Key high pass filter using RC-CR transformation. \hspace{11cm} [3] (\bo{70 Ch}, 79 Ba, 75 Ash)
			\end{enumerate}
...
\section{Sensitivity}
	...
	\subsection{Sensitivity and importance of sensitivity analysis}
		\begin{enumerate}
			\item What is sensitivity? \hfill [1] {\footnotesize (\bo{75 Ch, 74 Ch, 73 Ch}, 81 Ba, 73 Shr, 72 Ka, 71 Shr, 70 Asa)}

			\item What is sensitivity analysis in filter design? \hfill [1] (\bo{\texttt{81 Bh}})
			...
			\enter\hfill [1] \begin{footnotesize} (\bo{\texttt{79 Bh}, 76 Ch, 75 Ch, 73 Ch}, 76 Ash, 73 Shr, 72 Ka) \end{footnotesize} [2] \begin{footnotesize} (\texttt{81 Ba, 80 Ba}, 74 Ash) \end{footnotesize}

			\item What information do you get when the sensitivity of y with respect to x is 0.1?\hfill [1] (\bo{\texttt{80 Bh}})
			\lb ... w.r.t to x is 0.5? \hfill [1] (\bo{80 Bh})
			\lb ... w.r.t to x is 2. \hfill [2] (\bo{79 Bh})
			\lb ... w.r.t to x is 1. \hfill [1] (79 Ba)
			\lb ... w.r.t to x is -5? \hfill [1] (\bo{72 Ch})
			\lb ... w.r.t to x is -3? \hfill [1] (\bo{69 Ch})
		\end{enumerate}

	\subsection{Definition of single parameter sensitivity}
		\begin{enumerate}[noitemsep, topsep=0pt]
			\item Explain the single parameter and multi-parameter sensitivity. \hfill [1] (80 Asa) [2] (81 Ba) [4] (\bo{80 Bh})

			\item What is single parameter sensitivity. \hfill [1] (\bo{70 Ch})
		\end{enumerate}
	...
	\subsection{Sensitivity properties of biquads}
		\begin{enumerate}
			...
			\enter\hfill [3] {\footnotesize (\bo{73 Ch}, 72 Ka, 70 Asa)} [4] {\footnotesize (81 Ba) } [5] {\footnotesize (\texttt{81 Ba, 80 Ba})}
			\lb for center frequency w.r.t all elements. \hfill [4] {\footnotesize (\bo{76 Ch, 75 Ch, 72 Ch, 70 Ch})} [5] {\footnotesize (\bo{79 Bh})}

			\item Perform the sensitivity analysis of quality factor (Q) in Tow Thomas low pass biquad.
			\enter\hfill [2] {\footnotesize (71 Shr)} [4] {\footnotesize (\bo{\texttt{79 Bh}}, 80 Ba, 79 Ba, 73 Shr)} [5] {\footnotesize (\bo{\texttt{81 Bh}})} [6] {\footnotesize (\bo{81 Bh})}
			\lb of Q with respect to all the resistors and capacitors present in the circuit. 
			\enter\hfill[3] (\bo{\texttt{80 Bh}}) [4] (\bo{69 Ch}) [5] (76 Ash)

			\item Perform sensitivity analysis of low pass Tow Thomas biquad. \hfill [4] (\bo{74 Ch}, 74 Ash) [5] (\bo{78 Bh})

			\item Perform sensitivity analysis for center frequency (\w\sub{0}) and quality factor (Q) of the Tow Thomas lowpass filter w.r.t. all elements in the circuit. \hfill [5] (\bo{71 Ch})
		\end{enumerate}
...
\section{Design of High-Order Active Filters}
	...
	\begin{enumerate}[noitemsep, topsep=0pt]
		\item (Assumed) What is Bruton Transformation? \hfill [2] (\bo{\texttt{81 Bh}}) [3] (\bo{\texttt{79 Bh}})

		\item What is the importance of Bruton transformation? \hfill [2] (72 Ka)

		\item What are the different techniques of designing higher order active filters? Discuss briefly. \hfill [4] (70 Asa)
	\end{enumerate}

	\subsection{Cascade of biquads}
		\begin{enumerate}
			\item Design a 4\super{th} order LPF using cascaded two Sallenkey biquad having half power frequency of 1kHz and largest capacitor of 0.1$\mu$F. \hfill [7] (\bo{79 Bh})\
		\end{enumerate}

	\subsection{Active simulation of passive filters}
		\subsubsection{Ladder design with simulated inductors}
			\begin{enumerate}[topsep=0pt]
				\item What is GIC? \hfill [1] \begin{footnotesize} (\bo{\texttt{81 Bh}, 76 Ch, 70 Ch}, 81 Ba, 79 Ba, 75 Ash, 74 Ash, 73 Shr, 71 Shr) \end{footnotesize} [2] \begin{footnotesize} (\bo{79 Bh, 69 Ch}) \end{footnotesize}				 
				\lb What is gyrator? \hfill [1] (\bo{75 Ch})
				\lb What is Antonio's GIC? \hfill [1] (\bo{80 Bh})
				\lb What is ideal gyrator? \hfill [1] (\bo{74 Ch})

				\item Draw the ckt diagram of a GIC. \hfill [1] (\bo{72 Ch})

				\item Derive the relationship between i/p and o/p current of GIC. \hfill [2] (\bo{72 Ch})

				\item How can gyrator be used to simulate the grounded inductor in the passive filter? \hfill [4] (\bo{75 Ch, 74 Ch})
				\lb using GIC. \hfill [3] \begin{footnotesize} (\bo{79 Bh, 76 Ch}, 74 Ash) \end{footnotesize} [4] \begin{footnotesize} (\bo{81 Bh, 80 Bh, 70 Ch, 69 Ch}, 75 Ash, 73 Shr) \end{footnotesize} [5] \begin{footnotesize} (79 Ba, 71 Shr) \end{footnotesize}

				\item How can GIC be used to avoid shunt inductors in LC ladder circuit? \hfill [4] (\bo{\texttt{81 Bh}})
				...
				\enter\hfill [5] (\bo{79 Bh})
			\end{enumerate}
	
		\subsubsection{Ladder design with FDNR}
			\begin{enumerate}
				\item What is FDNR? \hfill [1] \begin{footnotesize} (\bo{\texttt{80 Bh}, 73 Ch, 71 Ch}, \texttt{80 Ba}, 80 Ba, 71 Shr, 70 Asa) \end{footnotesize} [2] \begin{footnotesize} (\bo{78 Bh}, 76 Ash) \end{footnotesize} 

				\item How can FDNR be realized? \hfill [3] (\texttt{80 Ba})

				\item How can GIC be used to simulate a grounded FDNR? \hfill [2] (\bo{72 Ch})
				\lb ...to simulate GIC? Explain with example. \hfill [4] (72 Ka)
				
				\item How can you use FDNR to avoid the inductor in filter design? 
				\enter\hfill [1] \begin{footnotesize} (71 Shr) \end{footnotesize} [2] \begin{footnotesize} (76 Ash, 70 Asa) \end{footnotesize} [3] \begin{footnotesize} (\bo{\texttt{80 Bh}}, 80 Ba) \end{footnotesize}
				\lb and explain from the circuit given below. \hfill [4] (\bo{73 Ch})\\
				...
				\item Draw the figure of RLC series circuit using FDNR. \hfill [2] (\bo{80 Bh})

				\item Design the LPF having \w\sub{0} = 10\super{4} rad/s and practical elements using FDNR. \hfill [5] (\bo{73 Ch})

				\item Design the 4\super{th} order Butterworth low pass filter with half power frequency 2,000 rad/sec and practically realizable elements using FDNR. \textit{[\hyperref[sec:tables_81bh]{Refer Table]}}. \hfill [4] (\bo{\texttt{81 Bh}})
				\lb for 20,000 rad/s. \hfill [5] (\bo{\texttt{79 Bh}, 78 Bh})
				\lb for 16,000 rad/s. \hfill [4] (\bo{76 Ch})
				\lb for 4,000 rad/s. \hfill [6] (\bo{72 Ch})

				\item Design third order Butterworth low pass filter having half power frequeuncy 4000rad/s using FDNR. \textit{[\hyperref[sec:tables_81bh]{Refer Table]}}.\hfill [6] (\bo{\texttt{80 Bh}})

				\item Use 4\super{th} order Butterworth lowpass by FDNR with half power 1000Hz and capacitor in final design = 0.01$\mu$F. \hfill [4] (\bo{80 Bh})\\
				...
				\lb for half power frequency 10,000 rad/s and practically realizable elements. \hfill [7] (\bo{74 Ch})
				...
				\item Following ckt is an HPF having half power frequency of 1 rad/s. Design an HPF having half power frequency of 4.5kHz by active simulation of inductors. In your final ckt the largest capacitance should be 0.1$\mu$F. \hfill [6] (\bo{69 Ch})\\
		...
		\subsubsection{Leapfrog simulation of ladders}
			\begin{enumerate}
				...
				\item Realize third order Butterworth LPF using Leapfrog simulation. \textit{[\hyperref[sec:tables_81bh]{Refer Table]}} \hfill [6] (\bo{70 Ch}, 79 Ba)
				\lb Design should have half power frequency of 4KHz and practically realizable elements. \hfill [6] (72 Ka)

				\item Design a 4\super{th} order Butterworth LPF having half power frequency of 4000 rad/s using Leapfrog simulation. \textit{[\hyperref[sec:tables_81bh]{Refer Table]}} \hspace{12.3cm} [6] (\bo{75 Ch})

				\item Simulate the Butterworth 4\super{th} order LPF in resistively terminated lossless network using Leapfrog active simulation referring information given in table below for LPF: \hfill [7] (\bo{81 Bh}) [8] (\texttt{81 Ba})\\
				\begin{tabular}{|c|c|c|c|c|c|c|}
					\hline
					Order(n)=4 and LPF & R$_1$ =1 & L$_1$=0.7654 & C$_2$=1.848 & L$_3$=1.848 & C$_4$ = 0.7654 & R$_2$=1\\ \hline
				\end{tabular}

				\item Using Leapfrog method to simulate the LC ladder ckt given below to obtain an LPF filter having passband of 6KHz and suitable element values. \hfill [6] (\bo{71 Ch})\\
				\includegraphics[width=4in]{./pics/fd_10}
			\end{enumerate}
...
\section{Switched-Capacitor Filters}
	...
	\subsection{The MOS switch and switched capacitor}
		\begin{enumerate}[topsep=0pt, noitemsep]
			...
			\enter\hfill [1] \begin{tiny} (\bo{\texttt{79 Bh}, 76 Ch, 72 Ch, 71 Ch, 70 Ch, 69 Ch}, \texttt{81 Ba}, 79 Ba, 76 Ash, 75 Ash, 74 Ash, 71 Shr) \end{tiny} [2] \begin{tiny} (\bo{80 Bh}) \end{tiny}

			\item What is the importance of switched capacitor filters? \hfill [2] (81 Ba)
			\lb What are its applications. \hfill [1] \begin{footnotesize} (\bo{\texttt{79 Bh}, 74 Ch, 70 Ch}, 79 Ba, 76 Ash) \end{footnotesize} [2] \begin{footnotesize} (\bo{73 Ch, 69 Ch}, 74 Ash, 71 Shr) \end{footnotesize}

			\item Design a switched capacitor filter to realize the magnitude response given by the plot below:
			\lb\includegraphics[width=0.5\textwidth]{./pics/fd_1} \hfill [6] (\bo{\texttt{81 Bh}})	[7] (\bo{81 Bh})
			\lb \includegraphics[width=3.5in]{./pics/fd_17} \hfill	[5] (\bo{75 Ch})
			\lb \includegraphics[width=3in]{./pics/fd_14} \hfill [5] (\bo{\texttt{79 Bh}}) [6] (\bo{78 Bh})
			...
			\lb \includegraphics[width=3.5in]{./pics/fd_16} \hfill [5] (79 Ba) [6] (\bo{79 Bh})
			\lb \includegraphics[width=3.5in]{./pics/fd_18} \hfill [5] (\bo{74 Ch, 73 Ch, 70 Ch}) [6] (\bo{72 Ch})
	...
	\subsection{Simulation of resistor by switched capacitor}
		\begin{enumerate}[topsep=0pt, noitemsep]
			...
			\enter\hfill [1] (\bo{\texttt{81 Bh}, 75 Ch}, 73 Shr) [2] (\bo{\texttt{80 Bh}})

			\item How can you simulate a resistor using switched capacitor? Explain w/ necessary derivations.
			\enter \hfill [3] (\bo{69 Ch}) [4] (\texttt{80 Ba}, 73 Shr)
		\end{enumerate}

	\subsection{Switched-capacitor circuits for analog operations: addition, subtraction, multiplication and integration}
		\begin{enumerate}[topsep=0pt, noitemsep]
			\item How summer, inverting integrator and non-inverting integrator can be realized using switched capacitor? Explain with necessary diagrams and expressions. [4] \begin{footnotesize} (\bo{\texttt{80 Bh}}) \end{footnotesize} [5] \begin{footnotesize} (\bo{76 Ch}, 76 Ash, 75 Ash) \end{footnotesize}			 
			...
			\item How inverting lossy integrator, integrator and non-inverting integrator can be realized usnig switched capacitor? Explain with necessary diagrams and transfer functions. \hfill [6] (\bo{71 Ch})
		\end{enumerate}

	\subsection{First-order and second-order switched-capacitor circuits}
		\begin{enumerate}[topsep=0pt, noitemsep]
			\item Design a switched capacitor filter to realize the transfer function: \hfill [5] (\bo{80 Bh}) [6] (\texttt{81 Ba}) [3+5] (72 Ka)\\
