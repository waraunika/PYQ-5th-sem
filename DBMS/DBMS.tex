...
\section{Relational Languages and Relational Model}
    ...
    \item Consider the relation: \hfill(\bo{\texttt{81 Bh}})
        ...
        \enter Project(\underline{pid}, pname, budget, location)
        \enter Work(empid, pid, responsibility, duration)
        \enter Payment(\underline{title}, salary)

        - SQL queries:  
        \begin{enumerate}[noitemsep, topsep = 0pt, label = \alph*.]
            \item Write SQL code to create the above relational schemas.
    ...
    \item Consider the relation:\hfill(\bo{75 Bh})
        ...
        \enter Enroll (enrollid, cname, courseid, enrolldate, completedate)
        
        - SQL queries:
        \begin{enumerate}[noitemsep, topsep = 0pt, label = \alph*.]
            \item create the above relations, including appropriate versions of all primary and foreign key integrity constraints
            \item to find crn, names and enroll date of all students who have taken the course `java' (cname)
            \item find the names and address of all the students who have taken both course java and linux.
            \item create a view 'student\_course' having the attributes crn, name, phone, coursename, enrolldate.
        \end{enumerate}
    ...
    \item Consider the relation:\hfill (\bo{74 Bh})
        ...
        \enter Works (\underline{empid}, \underline{deptid}, hours)

        - SQL query to
        \begin{enumerate}[noitemsep, topsep = 0pt, label = \alph*.]
            \item to create the above relations, including appropriate versions of all primary and foreign key integrity constraints.
    ...
    \item Consider the following relational schema\hfill[2x4] (\bo{\texttt{81 Bh}, 80 Ch})
        ...
        \begin{enumerate}[noitemsep, topsep = 0pt, label = \alph*.]
            ...
            \item Write the relational algebra to show the employee name along with their head name.\\
...
\section{Database Constraints and Normalization}
    ...
    \begin{enumerate}[noitemsep, topsep = 0pt]
        ...
        \item The set of functional dependencies is: (A$\ra$BC, CD$\ra$E, B$\ra$D, E$\ra$A), decomposed it into R1 = (A, B, C), R2 = (A, D, E). Show this decomposition is a lossless-join decomposition. \hfill [4] (\bo{\texttt{80 Bh}})
    ...
    \begin{enumerate}[noitemsep, topsep = 0pt]
        ...
        \lb What is the purpose of Normalization? \hfill [2] (\bo{77 Ch, 75 Bh}) [4] (70 Ma)
        ...
        \item How do you normalize a database to 1NF, 2NF, 3NF and 4NF? \hfill [6] (\bo{71 Bh})
        ...
        \enter\hfill [3] (70 Ma) [4] (\bo{75 Bh}) [6] (\bo{78 Ch})
...
\section{Query Processing and Optimization}
    \begin{center}(4 Hours/8 Marks)\end{center}
     \begin{enumerate}[topsep=0pt]
        ...
        \enter\hfill [2] \begin{footnotesize}(\bo{\texttt{80 Bh}})\end{footnotesize} [3] \begin{footnotesize}(\bo{78 Ch, 74 Bh})\end{footnotesize} [4] \begin{footnotesize}(\bo{76 Bh, 73 Bh})\end{footnotesize} [5] \begin{footnotesize}(\bo{77 Ch})\end{footnotesize} [6] \begin{footnotesize}(\bo{72 Ash, 68 Bh}, 68 Ma)\end{footnotesize}
     	...
     	\item What is the significance of materilazied views? \hfill [2] (\bo{72 Ash, 68 Bh})
  	\end{enumerate}
    ...
    \subsection{Evaluation of Expressions}
        \begin{enumerate}[topsep=0pt]
			...
			\lb Briefly explain pipelining approaches. \hfill [4] (\bo{\texttt{81 Bh}})
			\lb Explain with an example. \hfill [3] (\bo{79 Ch}, 76 Ba) 
            ...
            \lb Explain the difference between materialzation and pipelining methods for query evaluation 
			\enter\hfill [3] (71 Ma)
    ...
    \subsection{Query Optimization}
        \begin{enumerate}[topsep=0pt]
			...
			\item Explain the process of query optimization. \hfill [6] (\bo{69 Bh})
			\lb Explain how a DBMS chooses an apt query execution plan for optimized query execution.
			\enter\hfill [5] (71 Ma)
            ...
            a. \includegraphics[width=4in]{./pics/dbms_1} \hfill [5] (\texttt{81 Ba})\\
            b. \includegraphics[width=4in]{./pics/dbms_2} \hfill [2] (68 Ma)
            ...
            \item Transform the following relational algebra expression using equivalence rule. Show each step involved. \hfill [6] (\bo{\texttt{80 Bh}})\\
    			instructor (ID, name, dept\_name, salary)\\
    			teaches (ID, course\_id, sec\_id, semester, year)\\
    			course (course\_id, title, dept\_name, credits)\\
    			$\Pi_{\text{name, title}} \left( 
    \sigma_{\text{dept\_name}} = \text{``Music"} \char` \wedge \text{year} = 2017 \right.$
...
\section{File Structure and Hashing}
    ...
    \subsection{Disks, Storage, and RAID}
    		\begin{enumerate}[noitemsep, topsep=0pt]
    			\item What is RAID? \hfill [1] \begin{footnotesize}(\bo{\texttt{81 Bh}, 80 Ch, 77 Ch, 76 Bh, 71 Bh}, \texttt{80 Ba}, 73 Ma)\end{footnotesize} [3] \begin{footnotesize}(\bo{68 Bh}, 72 Ma)\end{footnotesize}
    			
			    \item What is the use of RAID storage devices? \hfill [2] (\bo{73 Bh}) [3] (\bo{70 Bh})
    			
    			\item Explain different levels of RAID. \hfill [3] (\bo{\texttt{81 Bh}}) [4] (\bo{80 Ch}) [6] (68 Ma)
    			...
    			\enter\hfill [3] \begin{footnotesize}(\bo{77 Ch, 76 Ch, 71 Bh})\end{footnotesize} [4] \begin{footnotesize}(\bo{79 Ch}, \texttt{80 Ba})\end{footnotesize}
    ...
    \subsection{Remote Backup System}
		\begin{enumerate}[noitemsep, topsep=0pt]
			\item Explain about remote back up system. \hfill [3] (\bo{79 Ch, 74 Bh, 70 Bh}, 76 Ba)
			\lb with diagram. \hfill [3] (\bo{73 Bh})

            \item Explain the architecture of remote backup system. \hfill [3] (\bo{77 Ch, 76 Bh, 71 Bh})
		\end{enumerate}		    
    ...
    \subsection{Order Indices}
    	\begin{enumerate}[noitemsep, topsep = 0pt]    			
            ...
            \enter\hfill [2] (\bo{76 Bh, 71 Bh}, \texttt{80 Ba}, 75 Ba)
            ...
            \enter\hfill [2] (\bo{79 Ch, 77 Ch}, 76 Ba) [3] (70 Ma) [5] (68 Ma)    
...
\section{Transactions processing and Concurrency Control}
    ...
    \subsection{Transactions and ACID}
            \begin{enumerate}[topsep=0pt]
                \item Define transaction. \hfill [1] (\bo{\texttt{79 Bh}, 80 Ch, 75 Bh}, 76 Ba, 71 Ma) [2] (72 Ma, \bo{\textit{68 Bh}})
                \lb and its properties. \hfill [3] (\bo{\texttt{79 Bh}, 80 Ch})
                ...
                \item What are the properties a transaction should satisfy in a database system? \hfill [3] (71 Ma)
                 \lb Explain ACID properties in database transaction. \hfill [4] (\bo{74 Bh}, 73 Ma) [5] (\textit{68 Ma})
                ...
                \lb What are the possible transaction states? \hfill [2] (\bo{76 Bh})
                \lb Explain different states of a transaction along with state transition diagram. \hfill [4] (\bo{72 Ash})
                ...
                \item Write short notes on Compensating Transaction. \hfill [4] (65 Ka)
            \end{enumerate}
    ...
    \subsection{Lock based Protocols}
         \begin{enumerate}[noitemsep, topsep=0pt]
            ...
            \item Explain two phase locking protocol for concurrency control in DBMS.
            \enter\hfill [4] (\bo{80 Ch, 78 Ch, 72 Ash}, 73 Ma)
...
\section{Crash Recovery}
    ...
    \subsection{Failure Classification}
        \begin{enumerate}
            \item What are the different types of failure in DBMS? \hfill [3] (\bo{\texttt{80 Bh}, 79 Ch, 75 Bh, 73 Bh})
        \end{enumerate}

    \subsection{Recovery and Atomicity}
        \begin{enumerate}
            \item Define term Recovery and Atomocity in database. \hfill [2] (76 Ba)

            \item (Assumed) What is stable storage? \hfill [2] (73 Ma)
            \lb Briefly explain the idea of stable storage. \hfill [3] (\bo{71 Bh})
        \end{enumerate}

    \subsection{Log-based Recovery}
        \begin{enumerate}
            \item Define a log based recovery method. \hfill [2] (\bo{\texttt{81 Bh}})

            \item Explain the idea of log-based recovery. \hfill [4] (73 Ma) [6] (\bo{72 Ash}, 75 Ba)

            \item How does log based recovery method work? \hfill [2] (\bo{80 Ch})

            \item Explain the conditions of using redo and undo operations during recovery process.
            \enter\hfill [4] (\bo{80 Ch}) [5] (\bo{\texttt{81 Bh}})

            \item Explain redo and undo phase of log based failure recovery mechanism. \hfill [6] (\bo{69 Bh}, 72 Ma)
            \lb How REDO and UNDO operations performed in log based based recovery mechanism?
            \enter\hfill [5] (\bo{\texttt{79 Bh}})

            \item Explain the functions of Undo and Redo operations in a log-based recovery of database.
            \enter\hfill [6] (70 Ma)

            \item What is deferred-database modification technique in context to log based recovery approach? Explain. \hfill [3] (\bo{76 Bh}) [4] (\texttt{81 Ba})

            \item Explain about immediate-database modification techniques with example.
            \enter\hfill [4] (\bo{77 Ch}) [5] (\texttt{81 Ba})

            \item Distinguish between immediate and deferred modification. \hfill [4] (71 Ma)

            \item Explain about checkpoints with example. \hfill [3] (\bo{\texttt{79 Bh}}, \texttt{81 Ba})

            \item How does the check point recovery take place in case of transaction failure? \hfill [4] (\bo{78 Ch})

            \item Explain the purpose of the checkpoint mechanism. \hfill [2] (\bo{74 Bh}, \texttt{81 Ba})
            \lb What is the significance of checkpoints in a log? \hfill [2] (71 Ma)

            \item How often should checkpoints be performed? \hfill [2] (\texttt{81 Ba})

            \item Consider the following log contents when a crash occurs. Explain how recovery would be done for each state. \hfill [4] (76 Ba)\\
            \includegraphics[width=4in]{./pics/dbms_3}

            \item What are the recovery actions performed if failure arises at the end of the given transaction states?\\
            \includegraphics[width=4.5in]{./pics/dbms_4} \hfill [4] (\bo{74 Bh})

            \item Suppose following contents are present in log when a crash occurs. Explain what happens for a log-based recovery. \hfill [6] (\bo{70 Bh})\\
            \includegraphics[width=1.5in]{./pics/dbms_5}
        \end{enumerate}

    \subsection{Shadow paging}
        \begin{enumerate}
            \item Explain shadow page recovery technique with example. \hfill [4] (\bo{78 Ch}) [5] (\bo{\texttt{80 Bh}})

            \item Differentiate between shadow paging and log-based recovery. \hfill [3] (\bo{75 Bh, 73 Bh})
        \end{enumerate}
...
\section{Advanced database Concepts}
    \begin{center}(4 Hours/6 Marks)\end{center}
    \subsection{Concept of Object-Oriented}
        \begin{enumerate}
            \item Describe briefly about object oriented database. \hfill [3] (\bo{69 Bh}, 73 Ma)

            \item What are differences between object oriented and relational database management system.
            \enter\hfill [3] (\texttt{81 Ba})

            \item Write short notes on: Features of Object Oriented Database. \hfill [3] (\bo{79 Ch})
            \lb Write short notes on: Advantages of Object Oriented Database model. \hfill [3] (\bo{\texttt{79 Bh}})
            \lb What is the significance of object-oriented databases? \hfill [3] (72 Ma)

            \item Write short notes on: Object Relational Mapping (ORM). \hfill [3] (\bo{77 Ch}, 71 Ma)
        \end{enumerate}

    \subsection{Distributed Database Model}
        \begin{enumerate}
            \item Write short notes on: Distributed Database System. \hfill [3] (\bo{\texttt{81 Bh}, 73 Bh}, 76 Ba)

            \item Explain Distributed Database System with its type. \hfill [3] (\bo{71 Bh}) [4] (\bo{\texttt{80 Bh}})

            \item Explain properites of distributed databases. \hfill [4] (\bo{70 Bh})

            \item Explain homogeneous and heterogeneous distributed database. \hfill [3] (73 Ma) [4] (75 Ba)

            \item What are different data fragmentation techniques? \hfill [2] (\bo{\texttt{80 Bh}})
            \lb What are the types of data fragmentation in distributed databases? \hfill [3] (\texttt{81 Ba})
            \lb Write short notes on: Fragmentation in distributed databases. \hfill [3] (\bo{77 Ch})

            \item Explain horizontal and vertical fragmentation in distributed databases. \hfill [3] (70 Ma)

            \item Write any four advantages of distributed database. \hfill [3] (\texttt{81 Ba})
        \end{enumerate}

    \subsection{Parallel Databases}
        \begin{enumerate}
            \item Write short notes on: Parallel databases \hfill [3] (\bo{\texttt{79 Bh}, 80 Ch, 79 Ch})
            \lb Write short notes on: Parallel database architecture. \hfill [3] (\bo{76 Bh}, 72 Ma, 71 Ma)

            \item Explain the benefit of parallel database. \hfill [3] (\bo{69 Bh})
        \end{enumerate}
        
    \subsection{Concept of Data warehouse Database}
        \begin{enumerate}
            \item Write short notes: Data warehouse. \hfill [3] (\bo{\texttt{81 Bh, 79 Bh}, 80 Ch, 76 Bh, 71 Bh})
            \lb Write short notes on data warehouse with its components. \hfill [4] (\bo{75 Bh})

            \item Write short notes on: Importance of Data warehouses. \hfill [3] (\bo{72 Ash}) [4] (\bo{78 Ch})

            \item Write short notes on Data warehouse and associated applications. \hfill [3] (70 Ma)

            \item Differentiate between data warehousing and data mining with an example. \hfill [3] (\texttt{81 Ba})
        \end{enumerate}

    \subsection{Concept of Spatial Database}
        \begin{enumerate}
            \item What is spatial data? \hfill [1] (\texttt{81 Ba}) [2] (75 Ba)

            \item Write short notes on: Spatial database. \hfill [2] (\bo{75 Bh}) [3] (\bo{74 Bh, 73 Bh}) [4] (\bo{78 Ch})

            \item Write the application areas of spatial database. \hfill [3] (\bo{72 Ash})

            \item How do you store spatial data in a database? \hfill [2] (\texttt{81 Ba})
        \end{enumerate}
